...
{
    string rucksackOne = rucksacks[i - 2];
    string rucksackTwo = rucksacks[i - 1];
    string rucksackThree = rucksacks[i];
    foreach(char item in rucksackOne)
    {
        if(rucksackTwo.ToCharArray().Contains(item) && rucksackThree.ToCharArray().Contains(item))
        {
